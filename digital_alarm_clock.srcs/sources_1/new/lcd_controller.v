...
module lcd_controller(
    ...
    case (command)
        0: if (count == 4) command <= 1; // go to next command command after 40 ms
                else command <= 0; // stay in wait command
                
        255: begin // stay in idle command
            if (num_curr != {bcd4, bcd3, bcd2, bcd1}) begin
                command <= 13;
                num_curr <= {bcd4, bcd3, bcd2, bcd1};
            end
        end
        default: command <= command + 1; // default command
    endcase
    end
...
always @ (posedge clk) begin
        case (command)
            ...
            default: begin // default command, do nothing
                rs <= 0; // command mode
                en <= 0; // enable low
            end
        endcase
